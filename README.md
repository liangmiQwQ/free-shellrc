...
- Installation stores one cleanup helper per managed profile in the operating system's persistent user-state directory under the downstream package name. The helper remains available after the package is removed and deletes itself after successful cleanup.
