...
The downstream application must call `shellrcGuard(import.meta.url, executable)` at the top of its complete entry, before other application logic. `executable` is the product's user-facing executable name, without arguments or shell syntax. It must not be a JavaScript entry path or a package-manager store path.

The guard locates the nearest named `package.json` and resolves `executable` from the current `PATH` to an absolute launcher path. Resolution considers only filesystem launchers, including `PATHEXT` candidates on Windows; shell aliases and functions are not candidates. Failure to find a launcher returns `EXECUTABLE_NOT_FOUND`.
...
The package name discovered by `shellrcGuard` identifies the owning product. The guard stores the entry path, package manifest path, and launcher path for `installShellrc`; callers cannot supply a separate product identity. The launcher path remains lexical and is not resolved through symbolic links because the link itself represents the current installation while its target can belong to a shared package store.
...
An installed block contains the opening marker, a warning not to edit the managed region, the shell-specific runtime availability check, the caller-provided command for that shell, a shell-specific self-removal routine, and the closing marker. The caller's command remains opaque and is inserted exactly as provided apart from converting its line endings to match the target file.
...
When a shell loads the block, it checks that the guarded entry and package manifest are files and that the stored launcher path still exists as a filesystem directory entry. A launcher can be a regular file or symbolic link. The check must inspect the launcher itself instead of following it to a package-store target.

The entry, manifest, and launcher form one condition. All three must exist before the block removes the restart marker and executes the caller-provided command. If any check fails, the package is considered uninstalled: the block skips the command and removes its complete managed region from that profile. This additional launcher check is required because package managers such as pnpm can retain entry and manifest files in a [global virtual store](https://pnpm.io/global-virtual-store) after removing the project-owned launcher.

The block never executes the launcher for availability checking and never searches `PATH` again. Runtime failures such as a missing or damaged Node.js installation therefore do not remove the block. Functions, aliases, and other command shadows created by the caller-provided command cannot change the stored launcher check, including when a profile is loaded repeatedly in the same shell.

A filesystem wrapper found first in `PATH` is itself the launcher. If that wrapper remains after the product behind it is removed, the library cannot prove uninstallation and leaves the managed block in place.

The availability check and cleanup routine are library-generated implementation details for Bash, Zsh, Fish, Windows PowerShell, and PowerShell 7. They must not rewrite or reinterpret the caller-provided command. Repeated installation with the same command and resolved launcher remains idempotent; a changed launcher replaces the managed block in place.
...
The application must place `shellrcGuard(import.meta.url, executable)` before other entry logic and stop when it returns a diagnostic so a missing launcher, unsupported shell, or pending restart stops the complete application consistently.
...
- Launcher-present, launcher-missing with retained package-store files, entry-missing, and manifest-missing branches of each shell-specific managed block.
- Unsupported current shells and the create, reject, and shell-load removal lifecycle of the first-install restart marker.

Filesystem tests use temporary directories and cover missing parents, unchanged writes, permissions, symbolic links, launcher resolution, repeated installation after command shadowing, and concurrent-change detection. Tests must never target a developer's actual shell profile.
...
Existing applications adopt it by calling `shellrcGuard(import.meta.url, executable)` at the top of their entry, returning with a nonzero exit code for diagnostics, and passing a shell-aware command factory to `installShellrc`. Application-specific wrapper and action protocols stay in their existing repositories.
